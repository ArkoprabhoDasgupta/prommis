...
def D_calculation(element_name, system_name, pH_value):
    ...
    def slope_calculation(m):
        return m.M == sum(m.a[e,s]*LR_parameters[e,s][0] for e in Element for s in System)
    
    @m.Constraint()
    def intercept_calculation(m):
        return m.B == sum(m.a[e,s]*LR_parameters[e,s][1] for e in Element for s in System)
