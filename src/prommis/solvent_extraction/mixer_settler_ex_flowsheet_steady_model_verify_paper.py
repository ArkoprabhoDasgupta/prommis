...
for z in m.fs.mixer_settler_ex.aqueous_settler[1].unit.length_domain:
    if z != 0:
        m.fs.mixer_settler_ex.aqueous_settler[1].unit.properties[
            0.0, z
        ].hso4_dissociation.deactivate()
# m = build_model(dosage, number_of_stages)
...
set_scaling_factor(
    ...
    .distribution_expression_constraint["Y"],
    1e-5,
)
...
for e in m.fs.leach_soln.component_list:
    if e not in ["H2O", "H", "HSO4", "SO4", "Cl", "Al", "Ca", "Fe", "Sc"]:
        percentage_extraction[e] = (
            (
                m.fs.mixer_settler_ex.mixer[1]
                .unit.mscontactor.organic[0, 1]
                .conc_mass_comp[f"{e}_o"]()
                * m.fs.mixer_settler_ex.mixer[1]
                .unit.mscontactor.organic[0, 1]
                .flow_vol()
...
print(
    m.fs.mixer_settler_ex.mixer[1].unit.mscontactor.aqueous[0.0, 1].pH_phase["liquid"]()
)
percentage_extraction
