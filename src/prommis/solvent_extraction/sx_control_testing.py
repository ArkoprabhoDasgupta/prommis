...
for t in m.fs.time:
    for e in Elements:
        for s in stage_number:
            ...
            m.fs.solex.distribution_coefficient[t, s, "aqueous", "organic", e] = 10**(a*2+b)
...
#     return m.fs.solex.distribution_coefficient[t, s, "aqueous", "organic", e] == 10**(a*1.542 + b)
...
m.fs.solex.mscontactor.aqueous_inlet_state[:].conc_mass_comp["H"].fix(100.75)
...
m.fs.solex.mscontactor.aqueous[0, :].conc_mass_comp["H"].fix(100.75)
