...
m.fs.feed_to_module = Arc(
    source=m.fs.feed_tank.outlet, destination=m.fs.membrane_module[1].feed_phase_inlet
)

m.fs.strip_to_module = Arc(
    source=m.fs.strip_tank.outlet, destination=m.fs.membrane_module[2].strip_phase_inlet
)

m.fs.module_feed_1_to_2 = Arc(
    source=m.fs.membrane_module[1].feed_phase_outlet,
    destination=m.fs.membrane_module[2].feed_phase_inlet,
)

m.fs.module_strip_2_to_1 = Arc(
    source=m.fs.membrane_module[2].strip_phase_outlet,
    destination=m.fs.membrane_module[1].strip_phase_inlet,
)
...
for t in m.fs.time:
    ...
    for s in number:
        for z in m.fs.membrane_module[s].feed_phase.length_domain:
            set_scaling_factor(
                m.fs.membrane_module[s].feed_phase.properties[t, z].conc_mol_comp["H"],
                1,
            )
            set_scaling_factor(
                m.fs.membrane_module[s].feed_phase.properties[t, z].pH_constraint,
                1e5,
            )
            # set_scaling_factor(
            ...
            # )
            set_scaling_factor(
                m.fs.membrane_module[s].feed_phase.material_flow_linking_constraints[
                    t, z, "liquid", "H"
                ],
                1e5,
            )
        for e in m.fs.mem_prop.component_list:
            set_scaling_factor(
                m.fs.membrane_module[s].feed_phase.properties[t, z].conc_mol_comp[e],
                1e5,
            )
            set_scaling_factor(
                m.fs.membrane_module[s].feed_phase.properties[t, z].conc_mass_comp[e],
                1e5,
            )
            set_scaling_factor(
                m.fs.membrane_module[s].strip_phase.properties[t, z].conc_mol_comp[e],
                1e5,
            )

            # for r in m.fs.membrane_module.r:
            #     set_scaling_factor(
            #         m.fs.membrane_module[s].conc_mol_membrane_comp[t, z, r, e],
            #         1e2,
            #     )
...
msx_units = [scaled_model.fs.membrane_module[1], scaled_model.fs.membrane_module[2]]

# tank_units = [m.fs.feed_tank, m.fs.strip_tank]
# msx_units = [m.fs.membrane_module[1], m.fs.membrane_module[2]]
...
wts = StoreSpec.value()
to_json(
    m, fname="membrane_solvent_extraction_2_separate.json", wts=wts, human_read=True
)
