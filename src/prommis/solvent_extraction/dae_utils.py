...
def is_derivative(var, wrt=None, require_all=True):
    ...
    if len(set_count) == len(wrt_set_count) and all(
        set_count[s] <= wrt_set_count[s] for s in wrt_sets
    ):
...
def generate_discretization_components_along_set(m, set_, active=True):
    ...
    for var in m.component_objects(pyo.Var, active=active):
        if isinstance(var, dae.DerivativeVar) and set_ in ComponentSet(
            ...
            if not active or con.active:
                for idx, var in var_map.items():
                    con_active = any(con.active for con in con_map[idx].values())
                    if con_active:
        ...
        if isinstance(var, dae.DerivativeVar) and set_ in ComponentSet(
            ...
            if not active or con.active:
                for idx, deriv in deriv_map.items():
                    con_active = any(con.active for con in disc_map[idx].values())
                    if con_active:
...
class DAEInterface(object):
    ...
    def __init__(self, model, time, indices=None, active=True):
        ...
        diff_deriv_disc_list = list(
            generate_diff_deriv_disc_components_along_set(model, time, active=active)
        )
    ...
    def get_subsystem_at_time(self, t, include_inequality=False):
        ...
        constraints = list(
            _filter_duplicates(
                con[t]
                for con in self._dae_cons
                if t in con
                and con[t].active
                and (include_inequality or isinstance(con[t].expr, EqualityExpression))
            )
        )
        ...
        variables = list(
            _filter_duplicates(var[t] for var in self._dae_vars if var[t] in var_set)
        )
    ...
    def _get_valid_diff_deriv_disc(self, t, variables, constraints):
        ...
        for state, deriv, disc in self._diff_deriv_disc_list:
            ...
            #    deriv[t] in var_block_map,
            #    disc[t] in con_block_map,
            # )
            if (
                state[t] in var_block_map
    ...
    def _get_differential_constraints(self, t, diff_deriv_disc_list, constraints):
        ...
        diff_eqns = get_constraints_containing_variables(deriv_var_data, constraints)
    ...
    def get_naive_differential_subsystem_at_time(self, t, include_inequality=False):
    ...
    def get_naive_algebraic_subsystem_at_time(self, t, include_inequality=False):
        # Var and Constraint data objects at this point in time.
        variables, constraints = self.get_subsystem_at_time(
            t, include_inequality=include_inequality
        )
        ...
        diff_vars = [
            var[t] for var, _, _ in self._diff_deriv_disc_list if var[t] in var_set
        ]
        diff_deriv_var_set = ComponentSet(deriv_vars + diff_vars)
        disc_eqns = [
            eq[t] for _, _, eq in self._diff_deriv_disc_list if eq[t] in con_set
        ]
...
DifferentialHelper = DAEInterface
...
def get_constraints_containing_variables(variables, constraints):
    ...
    filtered_constraints = [
        ...
        if any(
            var in var_set for var in identify_variables(con.expr, include_fixed=False)
        )
    ]
