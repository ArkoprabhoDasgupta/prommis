...
def model_buildup():
    ...
    m.fs.mem_prop.D_coeff.fix()

    return m


def element_optimize(m, element):
    ...
    m.fs.mem_prop.D_coeff[element].unfix()
    ...
    def objective_function(m):
        feed_deviation = abs(
            (
                value(
                    m.fs.membrane_module.feed_phase.properties[0, 1].conc_mass_comp[
                        element
                    ],
                )
                - value(
                    units.convert(
                        feed_outlet_value[element],
        ...
        strip_deviation = abs(
            (
                value(
                    m.fs.membrane_module.strip_phase.properties[0, 1].conc_mass_comp[
                        element
                    ],
                )
                - value(
                    units.convert(
                        strip_outlet_value[element],
                        to_units=units.mg / units.L,
                    ),
                )
            )
        ...
        return (feed_deviation + strip_deviation) / 2
...
# for e in ["Al", "Ca", "Fe", "La", "Ce", "Pr", "Nd", "Sm", "Gd", "Dy", "Y"]:
m = model_buildup()
e = "Pr"
element_optimize(m, e)
m.fs.mem_prop.D_coeff[e].pprint()
