...
class CompoundSolventExtractionData(UnitModelBlockData):
    ...
    def build(self):
        ...
        for i in self.elements:
            ...
            self.add_inlet_port(
                name=f"solvent_extraction_aqueous_settler[{i}].unit_inlet",
                block=self.solvent_extraction_aqueous_settler[i].unit,
            )
            self.add_outlet_port(
                name=f"solvent_extraction_aqueous_settler[{i}].unit_outlet",
                block=self.solvent_extraction_aqueous_settler[i].unit,
            )
        ...
        for i in self.elements:
            ...
            self.add_inlet_port(
                name=f"solvent_extraction_organic_settler[{i}].unit_inlet",
                block=self.solvent_extraction_organic_settler[i].unit,
            )
            self.add_outlet_port(
                name=f"solvent_extraction_organic_settler[{i}].unit_outlet",
                block=self.solvent_extraction_organic_settler[i].unit,
            )
