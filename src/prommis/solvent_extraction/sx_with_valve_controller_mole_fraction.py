...
m.x_o = Var(m.fs.time, stage_number, m.fs.prop_o.dissolved_elements, bounds=(0, 1))
...
def organic_x_addition(m, t, s):
    return sum(m.x_o[t, s, e] for e in m.fs.prop_o.dissolved_elements) == 1
...
@m.Constraint(m.fs.time, stage_number, m.fs.prop_o.dissolved_elements)
