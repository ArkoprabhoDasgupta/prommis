...
from pyomo.environ import ConcreteModel, units, TransformationFactory, RangeSet, value
    ...
    if e not in ["H2O", "H", "SO4", "HSO4", "Cl", "Ascorbic"]:
        percentage_recovery[e] = [
            (
                (
                    (
                        m.fs.scrub_sx.aqueous_outlet.conc_mass_comp[t, e]()
                        * m.fs.scrub_sx.aqueous_outlet.flow_vol[t]()
                        - m.fs.scrub_sx.aqueous_inlet.conc_mass_comp[t, e]()
                        * m.fs.scrub_sx.aqueous_inlet.flow_vol[t]()
                    )
                    + (
                        m.fs.strip_sx[
...
strip_scrub_ratio["tree"] = [
    (
        sum(
            m.fs.strip_sx[strip_number_of_stages].aqueous_outlet.conc_mass_comp[t, e]()
            * m.fs.strip_sx[strip_number_of_stages].aqueous_outlet.flow_vol[t]()
            for e in m.fs.leach_soln.component_list
            if e not in ["H2O", "H", "SO4", "HSO4", "Cl", "Ascorbic", "Fe"]
        )
